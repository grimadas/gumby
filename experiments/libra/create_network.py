...
def initial_node_startup(num_validators, root_dir):
    ...
    while current_node + STARTUP_BATCH_SIZE <= num_validators:
        ...
        for node_index in range(current_node, current_node + STARTUP_BATCH_SIZE):
            cmd = "/home/jenkins/diem/diem-node -f /tmp/diem_data_%d/%d/node.yaml > /tmp/diem_data_%d/logs/%d.log 2>&1" % (num_validators, node_index, num_validators, node_index)
...
if __name__ == "__main__":
    ...
    if os.path.exists(network_path):
        ...
        initial_node_startup(args.validators, network_path)
