...
class TrustchainModule(IPv8OverlayExperimentModule):
    def __init__(self, experiment):
        super(TrustchainModule, self).__init__(experiment, OrigTrustChainCommunity)
    ...
    def init_trustchain(self):
        ...
        self.block_times_file = os.path.join(os.environ['PROJECT_DIR'], 'output', 'block_times_'
                                             + str(self.my_id) + '.csv')
        with open(self.block_times_file, "w") as t_file:
            writer = csv.DictWriter(t_file, ['id', 'time'])
            writer.writeheader()
        with open(self.block_stat_file, "w") as t_file:
            writer = csv.DictWriter(t_file, ['time', 'transaction', 'type', "seq_num", "link", 'from_id', 'to_id'])
            writer.writeheader()
