...
LIBRARY_NAME = '/usr/local/lib/libnethogs.so.0.8.5-70-gd4f3b22'
...
def write_traffic_res(prefix):
    import csv
    csv_columns = ['process', 'total_up', 'total_down']

    csv_file = os.path.join(prefix, "traffic_res.csv")
    try:
        with open(csv_file, 'w') as f:
            writer = csv.DictWriter(f, fieldnames=csv_columns)
            writer.writeheader()
            for p, t in traffic_res.items():
                writer.writerow({'process': str(p),
                                 'total_up': t[0],
                                 'total_down': t[1]})
    except IOError:
        print("I/O error")
...
def write_traffic_times(prefix):
    import csv
    csv_columns = ['process', 'total_up', 'total_down', 'time']

    csv_file = os.path.join(prefix, "traffic_times.csv")
    try:
        with open(csv_file, 'w') as f:
            writer = csv.DictWriter(f, fieldnames=csv_columns)
            writer.writeheader()
            for p, t in traffic_vals.items():
                for v in t:
                    writer.writerow({'process': str(p),
                                     'total_up': v[0],
                                     'total_down': v[1],
...
def signal_handler(signal, frame):
    ...
    # Write to files
    prefix = ''
    write_traffic_res(prefix)
    write_traffic_times(prefix)
...
def network_activity_callback(action, data):
    ...
    inst_name = str(data.contents.name) + '_' \
                + str(data.contents.pid) + '_' \
                + str(data.contents.uid) + '_' \
                + str(data.contents.device_name.decode('ascii'))

    traffic_res[inst_name] = (data.contents.sent_bytes, data.contents.recv_bytes)
    traffic_vals[inst_name].append((data.contents.sent_bytes,
